...
(function (w, d) {
  ...
  function hydrateVersions(family, sel) {
    ...
    (family.versions || []).forEach(v => {
      const years = Array.isArray(v.years) && v.years.length ? " [{', '.join(map(str, v.years))}]" : '';
  ...
  function bindUI() {
    ...
    pSel.addEventListener('change', () => {
      if (w.EngineFamilyState) w.EngineFamilyState.power = Number(pSel.value || w.EngineFamilyState.power || 0) || null;
    });
